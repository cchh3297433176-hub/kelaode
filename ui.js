...
export const UI = {
    ...
    // 绑定搜索输入框，兼容中文/日文等输入法的组合事件（composition）
    // 问题背景：中文拼音输入需经过"组合中"阶段（compositionstart~compositionend），
    // 部分移动端浏览器/WebView在compositionend后不会补发input事件，
    // 导致最终确定的文字从未真正触发过搜索回调，看起来像"卡死不出结果"。
    // 而emoji/英文是一次性直接插入，走的是普通input事件，所以不受影响。
    bindSearchInput: function(inputEl, callback) {
        if (!inputEl) return;
        let isComposing = false;
        const debouncedCallback = this.debounce((val) => callback(val), 250);
        ...
        inputEl.addEventListener('compositionend', (e) => {
            isComposing = false;
            // 组合结束，强制立即触发一次搜索（不走debounce，避免用户等待）
            callback(e.target.value);
        });

        inputEl.oninput = (e) => {
            // 组合过程中（比如还在选拼音候选字）不触发搜索，避免用中间态乱码搜索、浪费性能
    ...
    async open() {
        ...
        this.bindSearchInput($('#wb-search-entry'), (val) => this.renderList(val));
        ...
        this.bindSearchInput($('#wb-manage-search'), (val) => this.renderManageView(val));
